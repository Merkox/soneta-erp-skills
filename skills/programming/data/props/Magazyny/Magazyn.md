...
| Licz | `bool` | tylko-odczyt |  | Określa, czy dla danego magazynu mają być liczone zasoby i obroty. |
| Lokalizacja | `Soneta.CRM.Lokalizacja` | bazodanowe | Lokalizacja magazynu | Lokalizacja magazynu. |
| MarzaUjemna | `Soneta.Magazyny.StanyUjemne` (enum) | bazodanowe |  | Określa sposób działania programu w przypadku rozchodu (sprzedaży) towaru z magazynu po cenie niższej niż cena przychodu (zakupu). |
...
| OkresDostepnosci | `FromTo` | bazodanowe, podlista |  | Określa, okres dostępności, w którym magazyn jest widoczny na filtrach. |
...
| PrawaDoMagazynow | `SubTable<Soneta.Handel.MagDefDokRight>` | podlista |  |  |
| RezerwacjePonizejStanu | `Soneta.Magazyny.StanyUjemne` (enum) | bazodanowe |  | Określa sposób działania programu w przypadku gdy ilość rezerwowanego towaru przekracza stan magazynowy. |
| RozchodyPoPrzychodach | `bool` | bazodanowe |  | Wymusza łączenie rozchodów tylko z przychodami, które są wcześniej. |
| StanyUjemne | `Soneta.Magazyny.StanyUjemne` (enum) | bazodanowe |  | Określa sposób działania programu w przypadku wystąpienia stanu ujemnego w magazynie. |
