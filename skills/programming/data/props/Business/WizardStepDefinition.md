...
| Code | `MemoText` | bazodanowe, podlista | Kod algorytmu | Kod algorytmu kalkulatora kroku kreatora |
...
| DefinedPageXML | `MemoText` | bazodanowe, podlista | Zakładka użytkownika | Określa typ zakładki definiowanej przez użytkownika opisującej krok (XML) |
| Description | `string` | bazodanowe | Opis | Opis kroku kreatora |
| DescriptionText | `MemoText` | bazodanowe, podlista | Opis | Opis kroku kreatora |
...
| IsDbTuple | `bool` | tylko-odczyt |  |  |
| IsListStep | `bool` | tylko-odczyt | Zakładka typu lista | Określa czy zakładka jest listą (lista lub lista z edycją na liście) |
...
| Operator | `App.Operator` | bazodanowe | Operator | Operator, który może wykonać krok |
| OperatorType | `Db.Wizard.WizardStepOperatorType` (enum) | bazodanowe | Typ operatora | Typ operatora wykonującego |
...
| PageForCombo | `string` |  |  |  |
| PageType | `System.Type` | tylko-odczyt | Typ standardowej zakładki | Określa typ standardowej zakładki |
| ParentType | `string` | bazodanowe | Typ obiektu kroku kreatora | Określa typ obiektu kroku kreatora |
| Path | `string` |  |  |  |
| PreviewMultiplier | `Db.Wizard.WizardPreviewMultiplier` (enum) | bazodanowe | Wielkość podglądu | Określa wielkość podglądu załącznika |
| Report | `string` | bazodanowe | Raportowa zakładka | Określa typ zakładki raportującej opisującej krok |
| RichEditStepInfo | `MemoText` | bazodanowe, podlista | Opis kroku | Określa opis (informację) na kroku typu RichEdit |
...
| StepType | `Db.Wizard.WizardStepType` (enum) | bazodanowe | Typ | Typ kroku kreatora |
| WizardDefinition | `Db.Wizard.WizardDefinition` | bazodanowe |  | Definicja kreatora powiązana z krokiem |
| WorkerTypesList | `System.Collections.Generic.List<Db.Wizard.ActionAttributeListViewItem>` | podlista |  |  |
| WorkersTypes | `MemoText` | bazodanowe, podlista | Typy czynności dla kroku | Typy czynności jakie zostały zarejestrowane dla kroku |

## Enumy

Dozwolone wartości typów enum użytych w polach powyżej (`wartość` — Tytuł).

### WizardPreviewMultiplier (`Db.Wizard.WizardPreviewMultiplier`)
- `Small` = 0 — Mały
- `Normal` = 1 — Normalny
- `Medium` = 2 — Średni
- `Large` = 3 — Duży

### WizardStepOperatorType (`Db.Wizard.WizardStepOperatorType`)
- `Current` = 0 — Aktualny
- `Selected` = 1 — Wybrany
- `Role` = 2 — Rola

### WizardStepType (`Db.Wizard.WizardStepType`)
