...
| CRUPeriod | `FromTo` | bazodanowe, podlista | Okres | Okres obowiązywania umowy CRU |
| CRUStatus | `Soneta.Core.CRU.ContractStatus` (enum) | bazodanowe | Status umowy | Status umowy CRU |
...
| CreatedBy | `ITaskUser` | bazodanowe, tylko-odczyt, iface-ref | Użytkownik | Użytkownik/operator który utworzył rekord |
| DisplayName | `string` | tylko-odczyt |  |  |
| Host | `Soneta.Core.CRU.ICRUContractHost` | bazodanowe, tylko-odczyt | Rekord powiązany | Rekord powiązany |
| Model | `MemoText` | bazodanowe, podlista | Model | Zserializowany model danych |
| RowStatus | `Soneta.Core.CRUContractRowStatus` (enum) | bazodanowe | Status rekordu | Status rekordu - draft lub opublikowana |
| RowType | `Soneta.Core.CRUContractRowType` (enum) | bazodanowe | Rodzaj rekordu | Rodzaj rekordu - umowa lub aktualizacja |
