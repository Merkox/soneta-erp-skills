...
- pola bazodanowe: 126
- pola kalkulowane (z klas biznesowych): 174

| Pole | Typ | Rodzaj | Tytuł | Opis |
|------|-----|--------|-------|------|
| BezRabatu | `bool` | bazodanowe |  | Określa czy dla pozycji ma być zastosowany rabat. |
| BruttoCy | `Soneta.Types.Currency` |  | Wartość/B | Wartość brutto pozycji bez względu na to, czy obliczenia są od wartości netto, czy brutto. |
| Cena | `Soneta.Types.DoubleCy` | bazodanowe |  | Cena pozycji może być wyrażona w walucie i może być netto lub brutto. |
| CenaBruttoPoRabacie | `Soneta.Types.DoubleCy` |  |  | Cena brutto po rabacie wyliczona z wartości. |
| CenaJednostkowa | `Soneta.Types.DoubleCy` |  | Cena jednostkowa | Cena w jednostce ilości na pozycji. |
| CenaJednostkowaPoRabacie | `Soneta.Types.DoubleCy` |  | Cena jednostkowa po rabacie | Cena w jednostce ilości na pozycji po rabacie. |
| CenaNettoPoRabacie | `Soneta.Types.DoubleCy` |  |  | Cena netto po rabacie wyliczona z wartości. |
| CenaPoRabacie | `Soneta.Types.DoubleCy` |  |  | Cena wyliczona przez podzielenie wartości pozycji przez ilość. |
| Ceny | `Soneta.Handel.PozycjaDokHandlowego.CenyPozycji` |  |  | Kolekcja informacji o cenach liczonych względem tej pozycji dokumentu. |
| Czas | `Soneta.Types.Time` | bazodanowe |  | Czas powstania pozycji dokumentu, odczytywany z czasu dokumentu. |
| Data | `Soneta.Types.Date` | bazodanowe |  | Data powstania pozycji dokumentu, odczytywany z daty dokumentu. |
| DataPrzecenOkresowych | `Soneta.Types.Date` | bazodanowe |  | Data jaką należy zastosować dla przeceny okresowej na dokumencie rozliczającym umowę. |
| DataRealizacjiElementuRozliczenia | `Soneta.Types.Date` |  |  |  |
| DefinicjaCeny | `Soneta.Towary.DefinicjaCeny` |  |  | Definicja ceny wg def. dokumentu lub indywidualna produktu. |
| DefinicjaPowstaniaObowiazkuVAT | `Soneta.Business.Row` | bazodanowe |  | Definicja powstania obowiązku VAT dla pozycji. |
| DefinicjaStawki | `Soneta.Core.DefinicjaStawkiVat` | bazodanowe |  | Definicja stawki VAT naliczonej do tej pozycji dokumentu. |
| DodanaAutomatycznie | `bool` | bazodanowe | Pozycja dodana automatycznie | Pozycja dodana automatycznie np. przez schematy opakowań. |
| Dokument | `Soneta.Handel.DokumentHandlowy` | bazodanowe, guided-parent |  | Dokument, do którego należy ta pozycja |
| DokumentDostawaPozycja | `Soneta.Handel.DokumentDostawa` | bazodanowe |  | Informacje o dostawie związane z pozycją dokumentu handlowego. |
| DokumentDostawaPozycja.Cykl | `Soneta.Core.DefinicjaCyklu` | bazodanowe |  | Określa cykl/częstotliwość terminu dostawy. |
| DokumentDostawaPozycja.Cykl.AlgorytmCykluInstance | `Soneta.Core.IAlgorytmCyklu` |  |  | Instancja skompilowanego kalkulatora. |
| DokumentDostawaPozycja.Cykl.Czas | `Soneta.Types.Time` | bazodanowe |  | Określa czas wystąpienia cyklu. |
...
| DokumentDostawaPozycja.Cykl.Dzien | `Soneta.Types.Date` |  |  | Data rozpoczęcia wykonywania cyklu. |
| DokumentDostawaPozycja.Cykl.DzienCzas | `System.DateTime` |  |  |  |
...
| DokumentDostawaPozycja.Cykl.Godziny | `string[]` |  |  | Wybrane godziny w trybie godzinowym. |
...
| DokumentDostawaPozycja.Cykl.IsAdvanced | `bool` |  |  |  |
| DokumentDostawaPozycja.Cykl.IsMultiSelect | `bool` |  |  |  |
| DokumentDostawaPozycja.Cykl.IsSingleSelect | `bool` |  |  |  |
| DokumentDostawaPozycja.Cykl.JestAlgorytm | `bool` |  |  |  |
| DokumentDostawaPozycja.Cykl.JestOpcjaZaawansowana | `bool` |  |  |  |
| DokumentDostawaPozycja.Cykl.JestTermin | `bool` |  |  |  |
...
| DokumentDostawaPozycja.Cykl.OkresCyklu | `Soneta.Core.DefinicjaCykluOkresCyklu` | bazodanowe, enum |  | Określa okres, w którym oczekiwane jest wystąpienie cyklu. |
| DokumentDostawaPozycja.Cykl.Options | `Soneta.Core.DefinicjaCykluOptions` | enum |  |  |
...
| DokumentDostawaPozycja.Cykl.Podglad | `Soneta.Core.ElementCyklu` |  |  |  |
| DokumentDostawaPozycja.Cykl.Poniedzialek | `bool` |  |  | Poniedziałek - dzień tygodnia wystąpienia cyklu tygodniowego. |
| DokumentDostawaPozycja.Cykl.PozycjaDnia | `Soneta.Core.DefinicjaCykluPozycjaDnia` | bazodanowe, enum |  | Określa pozycję dnia w okresie, w którym oczekiwane jest wystąpienie cyklu. |
| DokumentDostawaPozycja.Cykl.PozycjaDniaZaawansowana | `Soneta.Core.DefinicjaCykluPozycjaDniaZaawansowana` | bazodanowe, enum |  | Określa pozycję dnia w miesiącu, w którym oczekiwane jest wystąpienie cyklu. |
| DokumentDostawaPozycja.Cykl.RodzajTerminu | `Soneta.Core.DefinicjaCykluRodzajTerminu` | bazodanowe, enum |  | Wskazuje na to, czy określono termin cyklu lub termin wynika z opisu cyklu. |
...
| DokumentDostawaPozycja.Cykl.SposobNaDniWolne | `Soneta.Core.DefinicjaCykluSposobNaDniWolne` | bazodanowe, enum |  | Określa sposób zachowania, kiedy cykl wystąpi w dniu wolnym. |
...
| DokumentDostawaPozycja.Cykl.Typ | `Soneta.Core.DefinicjaCykluTyp` | bazodanowe, enum |  | Określa rodzaj cyklu za pomocą jednostki interwału czasu. |
...
| DokumentDostawaPozycja.Termin | `Soneta.Types.Date` | bazodanowe |  | Termin realizacji dostawy. |
| DokumentKorygowany | `string` |  |  |  |
...
| Dostawy | `Soneta.Business.SubTable<Soneta.Handel.PozycjaDokHandlowego>` |  |  |  |
| Dzien | `Soneta.Handel.UmowaDzien` | bazodanowe, enum |  |  |
| ElemyRozliczenia | `Soneta.Business.SubTable<Soneta.Handel.ElemRozliczenia>` |  |  |  |
| ExtInfo | `Soneta.Handel.ExternalInformation` |  |  |  |
...
| GrupaPKD | `Soneta.Handel.GrupaPKD` | enum | Grupa PKD |  |
| GrupaTowarowaVat | `Soneta.Core.ProceduraVAT` | bazodanowe |  |  |
| Ident | `int` | bazodanowe |  | Identyfikator pozycji na dokumencie handlowym. |
...
| IloscBiezacaZasobu | `Soneta.Towary.Quantity` |  | Ilość bieżąca zasobu | Suma ilości zasobów pozycji, w wartość bezwzględnej. |
...
| IloscNaObrotach | `Soneta.Towary.Quantity` |  | Ilość zarezerwowana | Zsumowana ilość danej pozycji na obrotach magazynowych. |
...
| IlośćTekst | `string` |  | Ilość | Ilość towaru wyrażona w postaci tekstu z uwzględnieniem jednostki reszty. |
| IlośćWartość | `Soneta.Handel.IlośćWartość` |  |  |  |
| IlośćWartośćMagazynu | `Soneta.Handel.IlośćWartość` |  |  |  |
| IlośćWartośćStatystyczna | `Soneta.Handel.IlośćWartość` |  |  |  |
| IlośćWartośćZasobu | `Soneta.Handel.IlośćWartość` |  |  |  |
| IlośćZasobu | `Soneta.Towary.Quantity` |  |  | Ilość zasobu przeniesiona do magazynu z uwzględnieniem pozycji podrzędnych. |
| IlośćŁącznaTowaru | `Soneta.Towary.Quantity` |  | Ilość łączna | Zsumowana ilość danego towaru znajdująca się na tym dokumencie wyrażona w jednostkach magazynowych. |
| IlośćŁącznaTowaruNaKorygowanym | `Soneta.Towary.Quantity` |  | Ilość łączna na dokumencie korygowanym | Zsumowana ilość danego towaru znajdująca się na dokumencie korygowanym wyrażona w jednostkach magazynowych. |
...
| JestPrzecenaPrzychodu | `bool` |  |  | Określa, czy dana pozycja dokumentu może przeceniać towar. |
| JestUnijny | `bool` |  |  | Wskazuje, czy towarem można handlować z kontrahentami unijnymi. |
| JestUprawnienieDoZmianyCeny | `bool` |  |  |  |
| KierunekMagazynu | `Soneta.Magazyny.KierunekPartii` | bazodanowe, enum |  | Określa kierunek wprowadzania partii towaru na magazyn. |
| Kod | `string` |  |  |  |
...
| KodIndywidualny | `string` |  |  | Indywidualny kod towaru dla kontrahenta lub kod z karty towaru. |
| KompletacjaInfo | `Soneta.Handel.KompletacjaInfo` | bazodanowe |  | Dane właściwe dla kompletacji. |
| KompletacjaInfo.Dodatkowa | `bool` | bazodanowe |  | Dodatkowa. |
| KompletacjaInfo.IloscKartotekowa | `Soneta.Towary.Quantity` |  | Ilość kartotekowa |  |
...
| KompletacjaInfo.Przychod | `Soneta.Handel.Kompletacje.PozycjaKompletacjaWorker` |  |  |  |
| KompletacjaInfo.Rozchod | `Soneta.Handel.Kompletacje.PozycjaKompletacjaWorker` |  |  |  |
| KompletacjaInfo.Wartosc | `Soneta.Types.Currency` | bazodanowe |  |  |
...
| Korekta | `bool` |  |  | Określa, czy dana pozycja koryguje inną pozycję. |
...
| Korygowana | `bool` |  |  | Sprawdza, czy pozycja dokumentu korygującego wprowadza jakieś zmiany w dokumencie korygowanym. |
...
| KrajPrzeznaczeniaNazwa | `string` |  |  | Pełna nazwa kraju przeznaczenia towaru wykazywana w deklaracji Intrastat. |
...
| Krotnosc | `Soneta.Handel.UmowaKrotnosc` | bazodanowe, enum |  |  |
| KwotaRabatu | `Soneta.Types.Currency` |  |  | Kwota udzielonego rabatu na pozycji. |
| LiczMagazyn | `bool` |  |  | Określa, czy ma być przeliczany ruch parii w magazynie. |
| LiczWgDostawy | `bool` |  |  | Określa, czy dla tej pozycji musi być określana dostawa partii towaru. |
...
| MasaJednostkowaNetto | `Soneta.Towary.Quantity` |  |  |  |
...
| Nadrzędne | `Soneta.Handel.PozycjaDokHandlowego.PozycjeSubTable.Podrzędne` |  |  |  |
| NadrzędneRelacje | `Soneta.Business.SubTable` |  |  |  |
| NaliczajRabatKaucji | `bool` | bazodanowe | Naliczaj rabat kaucji. | Naliczaj rabat kaucji. |
| Nazwa | `string` | bazodanowe |  |  |
| NazwaIndywidualna | `string` |  |  | Indywidualny nazwa towar dla kontrahenta lub nazwa z karty towaru. |
| NazwaObieLinie | `string` |  |  |  |
| NazwaPierwszaLinia | `string` |  |  |  |
| NazwaResztaLinii | `string` |  |  |  |
| NettoCy | `Soneta.Types.Currency` |  | Wartość/N | Wartość netto pozycji bez względu na to, czy obliczenia są od wartości netto, czy brutto. |
...
| Obroty | `Soneta.Business.SubTable` |  |  | Obroty związane bezpośrednio z pozycją dokumentu. |
| ObrotyWszystkie | `Soneta.Business.ListWithView` |  |  | Wszystkie obroty związane z pozycją dokumentu, łącznie z obrotami dokumentów zależnych (bez storno zasobu). |
| ObrotyWszystkieWgPartiiPierwotnej | `Soneta.Business.ListWithView` |  |  | Wszystkie obroty związane z pozycją dokumentu, łącznie z obrotami dokumentów zależnych i korygujących (bez storno zasobu). |
| OkresRozliczony | `Soneta.Types.FromTo` | bazodanowe |  | Okres jaki rozlicza pozycja. |
| Opis | `Soneta.Business.MemoText` | bazodanowe |  |  |
| OpisAnalityczny | `Soneta.Business.SubTable` |  |  |  |
| OstatniElement | `Soneta.Handel.ElemRozliczenia` |  |  |  |
| OstatniElementPlanowania | `Soneta.Handel.ElemRozliczenia` |  |  |  |
| OstatniElementRealizacji | `Soneta.Handel.ElemRozliczenia` |  |  |  |
| OstatniElementRozliczenia | `Soneta.Handel.ElemRozliczenia` |  |  |  |
| OstatniElementZawieszenia | `Soneta.Handel.ElemRozliczenia` |  |  |  |
| OstatniaKorekta | `Soneta.Handel.PozycjaDokHandlowego` |  |  | Ostatnia korekta danej pozycji dokumentu. Zawiera niekorygowane wartości z dokumentu. |
...
| PełnaNazwa | `string` |  |  |  |
| Podrzędne | `Soneta.Handel.PozycjaDokHandlowego.PozycjeSubTable.Nadrzędne` |  |  |  |
| PodrzędneRelacje | `Soneta.Business.SubTable` |  |  |  |
| PodstawaZwolnienia | `string` | bazodanowe |  | Podstawa prawna dla zastosowania stawki ZW. |
| PozycjaKSeF | `Soneta.Handel.KSeFPozycjaDokHan` |  |  |  |
| PozycjaKorygowana | `Soneta.Handel.PozycjaDokHandlowego` |  |  | Pozycja korygowanego dokumentu handlowego przez tę pozycję korekty. |
| PozycjaKorygowanaPierwsza | `Soneta.Handel.PozycjaDokHandlowego` |  |  | Pozycja pierwszego korygowanego dokumentu handlowego przez tę pozycję korekty. |
| PozycjaKorygująca | `Soneta.Handel.PozycjaDokHandlowego` |  |  | Pozycja korygująca tę pozycję dokumentu handlowego. |
| PozycjaKorygującaOstatnia | `Soneta.Handel.PozycjaDokHandlowego` |  |  | Pozycja ostatniego korygującego tę pozycję dokumentu handlowego. |
...
| PrzeterminowaneRozliczenieKaucji | `bool` |  |  |  |
| Rabat | `Soneta.Types.Percent` | bazodanowe |  | Procent rabatu naliczony do ceny lub wartości pozycji. |
| RabatCeny | `Soneta.Types.DoubleCy` | bazodanowe |  | Rabat pomniejszający kwotę ceny w walucie. |
| RelacjePartii | `Soneta.Business.SubTable<Soneta.Handel.Dostawy.RelacjaGrupyDostaw>` |  |  |  |
| Rezerwacja | `Soneta.Handel.PozycjaDokHandlowego` |  |  | Pozycja dokumentu rezerwacji, który rezerwuje towary dla tej pozycji dokumentu handlowego. |
| RodzajKorekty | `Soneta.Handel.RodzajKorektyPozycji` | enum |  | Określa jaka wartość pozycji dokumentu została skorygowana. |
| RodzajKorektyOpis | `string` |  |  | Określa jaka wartość pozycji dokumentu została skorygowana uzupełniając informację o zmianę ceny netto po rabacie. |
| RodzajKorektyOpisEnglish | `string` |  |  |  |
| RodzajTransakcji | `Soneta.Handel.KodRodzajuTransakcji` | bazodanowe, enum |  | Rodzaj transakcji raportowany na dokumencie Intrastat. |
| RodzajUmowy | `Soneta.Handel.RodzajUmowy` | bazodanowe, enum | Rodzaj umowy | Określa rodzaj umowy. |
| RodzajZamówieniaNaPozycji | `Soneta.Handel.RodzajZamowieniaNaPozycji` | enum |  |  |
| RoznicaDlaBlokadyZasobu | `Soneta.Towary.Quantity` |  |  |  |
| RoznicaWartośćFakturowa | `decimal` |  |  | Różnica wartości fakturowej pozycji korygowanej. |
| RoznicaWartośćMagazynowa | `decimal` |  |  | Różnica wartości magazynowej pozycji korygowanej. |
| RoznicaWartośćStatystyczna | `decimal` |  |  | Różnica wartości statystyczej pozycji korygowanej. |
| SWW | `string` | bazodanowe |  | Numer SWW/PKWiU dla pozycji naliczonej dla obniżonej stawki VAT inicjowany z karty towaru. |
| SchematGratisow | `Soneta.Towary.SchemOpakowan` | bazodanowe | Schemat gratisów | Schemat gratisów. |
...
| SekcjaDokumentu | `Soneta.Handel.SekcjaDokumentu` | enum |  | Określa sekcję na wydruku, w której znajduje się dana pozycja. |
...
| StanRozliczonyElementuRozliczenia | `Soneta.Towary.Quantity` |  |  |  |
| StanZrealizowanyElementuRozliczenia | `Soneta.Towary.Quantity` |  |  |  |
| StatusPozycji | `Soneta.Handel.StatusPozycji` | bazodanowe, enum |  | Określa status pozycji dokumentu handlowego. |
| Stawka | `Soneta.Core.StawkaVat` | bazodanowe |  | Rodzaj stawki VAT, która została naliczona do tej pozycji dokumentu. |
...
| Suma | `Soneta.Handel.BruttoNetto` | bazodanowe |  | Przeliczone na PLN i odpowiednio policzone wartości Netto, Brutto i VAT |
...
| WOkresie | `Soneta.Handel.UmowaWOkresie` | bazodanowe, enum |  |  |
| WartoscCy | `Soneta.Types.Currency` | bazodanowe | Wartość | Ostateczna wartość pozycji wyrażona w walucie zależnej od wpisanej waluty ceny. |
| WartoscKGO | `Soneta.Types.DoubleCy` |  | Wartość KGO |  |
| Wartość | `decimal` |  |  | Wartość pozycji w walucie systemowej. |
...
| WartośćPrzedRabatem | `Soneta.Types.Currency` |  |  | Wartość pozycji przed pomniejszeniem o kwotę rabatu. |
...
| WartośćWCenieZakupuBufor | `decimal` |  |  | Wartość w cenie zakupu również dla niezatwierdzonych dokumentów. Dla korekt zmiana wartości zakupu. |
...
| WartośćŁącznaTowaru | `Soneta.Types.Currency` |  |  | Wartość łączna towaru na dokumencie. |
| WidoczneRoznice | `bool` |  |  |  |
...
| WspolczynnikWzor | `string` |  |  |  |
| ZaliczkaInfo | `Soneta.Handel.ZaliczkaInfo` | bazodanowe |  |  |
| ZaliczkaInfo.Brutto | `decimal` |  | Zaliczka bieżąca (PLN) | Zaliczka bieżąca (PLN) |
| ZaliczkaInfo.BruttoCy | `Soneta.Types.Currency` | bazodanowe | Zaliczka bieżąca | Kwota brutto tej części zaliczki, która przypada na bieżącą pozycję dokumentu (z kompensacją zaokrągleń). |
| ZaliczkaInfo.BruttoCyNarastajaco | `Soneta.Types.Currency` |  | Zaliczka bieżąca łącznie | Łączna kwota dotychczasowych zaliczek brutto, która przypada na bieżącą pozycję dokumentu (z kompensacją zaokrągleń). |
| ZaliczkaInfo.NettoCy | `Soneta.Types.Currency` |  | Zaliczka bieżąca /N | Kwota netto tej części zaliczki, która przypada na bieżącą pozycję dokumentu (z kompensacją zaokrągleń). |
| ZaliczkaInfo.NettoCyNarastajaco | `Soneta.Types.Currency` |  | Zaliczka bieżąca /N łącznie | Łączna kwota dotychczasowych zaliczek netto, która przypada na bieżącą pozycję dokumentu (z kompensacją zaokrągleń). |
| Zasoby | `Soneta.Business.SubTable` |  |  | Zasoby na stanie magazynowym związane bezpośrednio z pozycją dokumentu. |
| ZasobyWszystkie | `Soneta.Business.ListWithView` |  |  | Wszystkie zasoby na stanie magazynowym związane z pozycją dokumentu, łącznie z zasobami pozycji dokumentów zależnych. |
| ZasobyWszystkieWgPartiiPierwotnej | `Soneta.Business.ListWithView` |  |  | Wszystkie zasoby na stanie magazynowym związane z pozycją dokumentu, łącznie z zasobami pozycji dokumentów zależnych. |
| Zmiana | `Soneta.Handel.IlośćWartość` |  |  | Dla dokumentu korygowanego zwraca korektę ilości i wartości pozycji, ale pozostałych ilość i wartość pozycji towaru. |
| ZmianaBrutto | `decimal` |  |  | Dla dokumentu korygowanego zwraca korektę wartości brutto pozycji, ale pozostałych wartość brutto pozycji towaru. |
| ZmianaCeny | `Soneta.Types.DoubleCy` |  |  | Dla dokumentu korygowanego zwraca korektę ceny pozycji. |
| ZmianaCenyNettoPoRabacie | `Soneta.Types.DoubleCy` |  |  | Dla dokumentu korygowanego zwraca korektę ceny netto po rabacie pozycji. |
| ZmianaIlości | `Soneta.Towary.Quantity` |  |  | Dla dokumentu korygowanego zwraca korektę ilości pozycji, ale pozostałych ilość towaru. |
| ZmianaIlościMagazynu | `Soneta.Towary.Quantity` |  |  | Ilość (w jednostkach magazynowych) zmieniana przez tą pozycję. |
| ZmianaIlościTekst | `string` |  | Zmiana ilości | Zmiana ilości (właściwość ZmianaIlości) towaru wyrażona w postaci tekstu z uwzględnieniem jednostki reszty. |
| ZmianaKwotyRabatu | `Soneta.Types.Currency` |  |  | Dla dokumentu korygowanego zwraca korektę kwoty rabatu pozycji, dla pozostałych kwotę rabatu. |
| ZmianaMagazynu | `Soneta.Handel.IlośćWartość` |  |  | Ilość (w jednostkach magazynowych) i wartość magazynu zmieniana przez tą pozycję. |
| ZmianaNetto | `decimal` |  |  | Dla dokumentu korygowanego zwraca korektę wartości netto pozycji, ale pozostałych wartość netto pozycji towaru. |
...
| ZmianaVAT | `decimal` |  |  | Dla dokumentu korygowanego zwraca korektę podatku VAT pozycji, ale pozostałych podatku VAT pozycji towaru. |
| ZmianaWartości | `decimal` |  |  | Dla dokumentu korygowanego zwraca korektę wartości pozycji, ale pozostałych wartość pozycji towaru. |
| ZmianaWartościCy | `Soneta.Types.Currency` |  |  | Dla dokumentu korygowanego zwraca korektę wartości w walucie pozycji, ale pozostałych wartość pozycji towaru. |
| ZmianaWartościCy2 | `Soneta.Types.Currency` |  |  | Wartość naliczana tylko dla raportu pozycje dokumentów.aspx Dla dokumentu korygowanego zwraca korektę wartości w walucie pozycji, ale pozostałych wartość pozycji towaru. |
