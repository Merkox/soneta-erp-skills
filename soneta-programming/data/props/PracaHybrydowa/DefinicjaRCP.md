...
| AlgorytmAkcji | `Soneta.PracaZdalna.RCP.Models.Database.Algorytm` (subrow) | bazodanowe |  | Algorytm akcji dla definicji rejestracji czasu pracy |
| AlgorytmAkcji.Aktywny | `bool` | bazodanowe |  | Określa czy istnieje algorytm dla definicji rejestracji czasu pracy |
| AlgorytmAkcji.Kod | `MemoText` | bazodanowe, podlista | Kod algorytmu | Kod algorytmów dla rejestracji czasu pracy |
...
| BlokadaEdycji | `Soneta.PracaZdalna.RCP.Models.Database.BlokadaEdycji` (subrow) | bazodanowe |  | Edytuj wyniki |
...
| BlokadaEdycji.RodzajDni | `Soneta.PracaZdalna.RCP.Enums.TypeOofDaysToCalculate` (enum) | bazodanowe | Dni wolne | Określa, w jaki sposób uwzględnić dane |
| BlokadaEdycji.WyliczWgKalend | `bool` | bazodanowe | Wylicz według dni z kalendarza | Czy wyliczyć według dni z kalendarza |
| BlokadaEdycji.Zakres | `Soneta.PracaZdalna.RCP.Enums.DataCollectionScope` (enum) | bazodanowe | Zakres | Zakres wykonywania operacji na danych, np. wyświetlanie, edycja |
| BlokadaWyswietlenia | `Soneta.PracaZdalna.RCP.Models.Database.BlokadaWyswietlenia` (subrow) | bazodanowe |  | Wyświetl wyniki |
...
| BlokadaWyswietlenia.RodzajDni | `Soneta.PracaZdalna.RCP.Enums.TypeOofDaysToCalculate` (enum) | bazodanowe | Dni wolne | Określa, w jaki sposób uwzględnić dane |
| BlokadaWyswietlenia.WyliczWgKalend | `bool` | bazodanowe | Wylicz według dni z kalendarza | Czy wyliczyć według dni z kalendarza |
| BlokadaWyswietlenia.Zakres | `Soneta.PracaZdalna.RCP.Enums.DataCollectionScope` (enum) | bazodanowe | Zakres | Zakres wykonywania operacji na danych, np. wyświetlanie, edycja |
| ClassName | `string` | tylko-odczyt |  |  |
| DziennyCzasPracy | `Time` | bazodanowe |  | Dzienny czas pracy |
| KodAlgorytmuAkcji | `MemoText` | podlista |  |  |
