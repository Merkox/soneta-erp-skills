...
- **Formularze — najpierw INDEX.** Katalog **wszystkich zakładek** (pageform) jest wyeksportowany do [`data/forms/INDEX.md`](data/forms/INDEX.md) (`Typ danych | Źródło | Nazwa bazowa | Zakładka | Priority | Biblioteka | Przestrzeń`, **posortowany po typie danych** — `DataType` gdy jest, inaczej nazwa bazowa). Odczyt natychmiastowy, bez DLL — do ustalenia „obiekt → zakładki". Regeneracja: `export-forms-index.csx`. Szczegóły: [references/scan-forms.md](references/scan-forms.md).
- `scan-forms.csx` — skaner **na żądanie** dla pojedynczego obiektu: odczytuje z zasobów osadzonych (`*.pageform.xml`/`*.form.xml`) **zakładki, sekcje danych (grupy) i pola** w **kolejności wprowadzania**, rozwijając ścieżki pól przez łańcuch `DataContext`/`EditValue`, dołączane `Include` (także między bibliotekami) oraz listy (`Grid`/`Scheduler`/… → kolumny elementu kolekcji, marker `[]`). Argument = prefiks nazwy pliku lub `DataType`/`Namespace.Typ`. Do budowania danych kodem i importu XML `business="true"`. Szczegóły: [references/scan-forms.md](references/scan-forms.md).
