...
| Zakładki, sekcje danych i pola formularzy (zasoby `*.pageform.xml`/`*.form.xml`) z DLL — rozwijanie ścieżek `DataContext`/`EditValue`, `Include`, kolejność pól pod kod i import XML | [references/scan-forms.md](references/scan-forms.md) |
...
- **Moduły i tabele — gotowy przegląd w [`data/props/INDEX.md`](data/props/INDEX.md).** To zarazem indeks propsów i pełna inwentaryzacja: moduł (z `Opis`) → tabele `RowType | Tytuł | Tabela | Konfig | Guided | Interfaces | Plik`. Odczyt natychmiastowy, dobre na start i do znalezienia `RowType`/`TableType`.
...
- `scan-forms.csx` — odczytuje z zasobów osadzonych (`*.pageform.xml`/`*.form.xml`) **zakładki, sekcje danych (grupy) i pola** formularza w **kolejności wprowadzania**, rozwijając ścieżki pól przez łańcuch `DataContext`/`EditValue` i dołączane `Include` (także między bibliotekami). Argument = prefiks nazwy pliku formularza (`Kontrahent`, `Dokument`). Użyteczny do budowania danych kodem i importu XML `business="true"`, gdzie kolejność i przynależność pól do sekcji ma znaczenie. Szczegóły: [references/scan-forms.md](references/scan-forms.md).
