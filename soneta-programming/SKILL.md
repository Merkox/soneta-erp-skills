...
- **Formularze — najpierw INDEX.** Katalog **wszystkich zakładek** (pageform) jest wyeksportowany do [`data/forms/INDEX.md`](data/forms/INDEX.md) (`Nazwa bazowa | Zakładka | Priority | DataType | Biblioteka | Przestrzeń`). Odczyt natychmiastowy, bez DLL — do ustalenia „obiekt → zakładki". Regeneracja: `export-forms-index.csx`. Szczegóły: [references/scan-forms.md](references/scan-forms.md).
